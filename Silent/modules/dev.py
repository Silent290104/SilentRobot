...
from Silent import DEV_USERS, OWNER_ID, dispatcher, telethn
...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        state = "off" if Silent.ALLOW_CHATS else "ʟᴏᴄᴋᴅᴏᴡɴ ɪs " + "on"
        update.effective_message.reply_text(f"ᴄᴜʀʀᴇɴᴛ sᴛᴀᴛᴇ: {state}")
        ʀᴇᴛᴜʀɴ
    if args[0].lower() in ["off", "no"]:
        Silent.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        Silent.ALLOW_CHATS = False
    else:
...
def leave(update: Update, context: CallbackContext):
    ...
    if args := context.args:
        ...
        update.effective_message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(Silent_leave_bt),
        )
...
def restart(update: Update, context: CallbackContext):
    ...
    process = subprocess.run("pkill python3 && python3 -m Silent", shell=True, check=True)
    process.communicate()
