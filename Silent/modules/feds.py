...
import Silent.modules.sql.feds_sql as sql
from Silent import DRAGONS, EVENT_LOGS, LOGGER, OWNER_ID, TIGERS, WOLVES, dispatcher
from Silent.modules.disable import DisableAbleCommandHandler
from Silent.modules.helper_funcs.alternate import send_message
from Silent.modules.helper_funcs.chat_status import is_user_admin
from Silent.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from Silent.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if fednam != "":
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't ғᴇᴅᴇʀᴀᴛᴇ! ᴘʟᴇᴀsᴇ ᴄᴏɴᴛᴀᴄᴛ @Silent_Smile_04 if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @Silent_Smile_04 should this problem persist!"
            )
            return
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Silent_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Silent_fbanned_users.json",
