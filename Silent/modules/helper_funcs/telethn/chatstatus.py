...
from Silent import DRAGONS
from Silent.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Silent_is_admin(chat_id: int):
    status = False
    Silent = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id,
        filter=ChannelParticipantsAdmins,
    ):
        if Silent.id == user.id:
            status = True
            break
    return status
