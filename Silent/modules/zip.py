...
from Silent import TEMP_DOWNLOAD_DIRECTORY
from Silent import telethn as client
...
async def _(event):
    ...
    if not event.is_reply:
        await event.reply("ʀᴇᴘʟʏ ᴛᴏ a ғɪʟᴇ to ᴄᴏᴍᴘʀᴇss ɪᴛ.")
        return
    if event.is_group:
        if not (await is_register_admin(event.input_chat, event.message.sender_id)):
            await event.reply(
                "Hey, ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ. ʏᴏᴜ ᴄᴀɴ'ᴛ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ, ʙᴜᴛ ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ɪɴ ᴍʏ ᴅᴍ🙂"
            )
            return
...
async def _(event):
    ...
    if event.is_group:
        if not (await is_register_admin(event.input_chat, event.message.sender_id)):
            await event.reply(
                "ʜᴇʏ, ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ. ʏᴏᴜ ᴄᴀɴ'ᴛ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ, ʙᴜᴛ ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ɪɴ ᴍʏ ᴅᴍ 🙂"
            )
            return
