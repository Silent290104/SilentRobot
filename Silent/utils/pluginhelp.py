...
from Silent import OWNER_ID, SUPPORT_CHAT, pgram
from Silent.utils.errors import split_limits
...
def time_formatter(milliseconds: int) -> str:
    ...
    tmp = (
        ...
        + ((str(minutes) + " ᴍɪɴᴜᴛᴇ(s), ") if minutes else "")
        + ((str(seconds) + " sᴇᴄᴏɴᴅ(s), ") if seconds else "")
        + ((str(milliseconds) + " ᴍɪʟʟɪsᴇᴄᴏɴᴅ(s), ") if milliseconds else "")
    )
    return tmp[:-2]
...
async def convert_to_image(message, client) -> [None, str]:
    """ᴄᴏɴᴠᴇʀᴛ ᴍᴏsᴛ ᴍᴇᴅɪᴀ ғᴏʀᴍᴀᴛs ᴛᴏ ʀᴀᴡ ɪᴍᴀɢᴇ"""
    final_path = None
    if not (
